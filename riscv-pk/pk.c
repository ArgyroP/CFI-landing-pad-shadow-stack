...
{
  size_t mem_pages = mem_size >> RISCV_PGSHIFT; //RISCV_PGSHIFT 12
  ...
  printk("current.mmap_max: 0x%lx\n", current.mmap_max);
  current.stack_top = stack_bottom + stack_size;
  printk("current.stack_top: 0x%lx\n", current.stack_top);
  ...
  size_t shadow_stack_bottom = __do_mmap(stack_bottom - stack_size, stack_size, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANONYMOUS|MAP_FIXED, 0, 0);
 
  kassert(shadow_stack_bottom != (uintptr_t)-1);
  
  current.stack_top = shadow_stack_bottom + stack_size; 
  printk("Shadow Stack bottom: 0x%lx\n", shadow_stack_bottom);
  printk("Shadow Stack top: 0x%lx\n", current.stack_top);
  size_t shadow_stack_top = current.stack_top - current.phdr_size;
  memcpy_to_user((void*)shadow_stack_top, (void*)current.phdr, current.phdr_size);
  current.phdr = shadow_stack_top;
  
  for (size_t i = 0; i < argc; i++) {
    printk("argc: 0x%lx\n",argc);
    size_t len = 12;//strlen((char*)(uintptr_t)argv[i])+1;
    shadow_stack_top -= len;
    memcpy_to_user((void*)shadow_stack_top, (void*)(uintptr_t)argv[i], len);    
    argv[i] = (void*)shadow_stack_top;
  }
    for (size_t i = 0; i < envc; i++) {
    size_t len = strlen(envp[i]) + 1;
    shadow_stack_top -= len;
    memcpy_to_user((void*)shadow_stack_top, envp[i], len);
    envp[i] = (void*)shadow_stack_top;
  }

  #define SHADOW_STACK_INIT(type) do { \
    unsigned naux = sizeof(aux)/sizeof(aux[0]); \
    shadow_stack_top -= (1 + argc + 1 + envc + 1 + 2*naux) * sizeof(type); \
    shadow_stack_top &= -16; \
    type *sp = (void*)shadow_stack_top; \
    PUSH_ARG(int, argc); \
    for (unsigned i = 0; i < argc; i++) \
      PUSH_ARG(type, argv[i]); \
    PUSH_ARG(type, 0); /* argv[argc] = NULL */ \
  ...
  tf.ssp = shadow_stack_bottom + stack_size;
  __riscv_flush_icache();
  write_csr(sscratch, kstack_top);
  printk("enter Stack size: 0x%lx\n",stack_size);
  //write_csr(0x011,shadow_stack_bottom + stack_size);
  //asm volatile ("csrw 0x011, 0x0000003fff800000");
  printk("CSR!!!!!: ");
  start_user(&tf);
